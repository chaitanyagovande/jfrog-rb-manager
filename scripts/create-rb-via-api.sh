#!/bin/bash
###############################################################################
# Create Release Bundle v2 via REST API
#
...
# Release Bundle v2 using the Lifecycle REST API.
#
# Prerequisites:
...
#
# Usage:
#   ./create-rb-via-api.sh <build-name-1> [build-name-2] [build-name-3] ...
#
...
#   DRY_RUN       - If "true", print payload but don't create RB (default: false)
...
echo "============================================================"
echo " Release Bundle v2 via REST API"
echo "============================================================"
...
# ─── Step 1: Construct the AQL query ────────────────────────────────────────
OR_ITEMS=""
...
# ─── Step 3: Build the Release Bundle API payload ───────────────────────────
echo ""
log "[3/4] Constructing Release Bundle payload..."

SOURCE_BUILDS=$(printf '%s' "$LATEST_BUILDS" | jq --argjson deps "$INCLUDE_DEPS" '
    [.[] | {
...
if [[ -n "$SIGNING_KEY" ]]; then
    PAYLOAD=$(jq -n \
        --arg rb_name "$RB_NAME" \
        --arg rb_version "$RB_VERSION" \
        --arg keypair "$SIGNING_KEY" \
        --argjson builds "$SOURCE_BUILDS" \
        '{
            release_bundle_name: $rb_name,
            release_bundle_version: $rb_version,
...
# ─── Step 4: Create the Release Bundle ──────────────────────────────────────
echo ""
if [[ "$DRY_RUN" == "true" ]]; then
    log "[4/4] DRY RUN — skipping Release Bundle creation."
    echo "       Payload above is what would be sent."
    exit 0
fi

log "[4/4] Creating Release Bundle v2..."
RESPONSE=$(jf rt curl -s -X POST \
    -H "Content-Type: application/json" \
    -d "$PAYLOAD" \
    /lifecycle/api/v2/release_bundle)

validate_json_response "$RESPONSE" "Release Bundle creation"
